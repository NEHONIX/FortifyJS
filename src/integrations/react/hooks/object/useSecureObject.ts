...
import { createSecureObject } from "../../../../security/secure-object";
import type { 
...
export function useSecureObject<T extends Record<string, any>>(
    ...
    useEffect(() => {
        if (!securityContext.config.development?.enablePerformanceMetrics) return;
        ...
        return () => {
            ...
            if (duration > 100) { // Log slow operations
                console.debug(
                    `useSecureObject: Component lifecycle took ${duration.toFixed(2)}ms`
                );
            }
        };

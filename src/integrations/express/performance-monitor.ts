...
export class PerformanceMonitor {
    ...
    public getMiddleware() {
        return (req: any, res: any, next: any) => {
            ...
            res.on('finish', () => {
                ...
                this.metrics.responses.min = Math.min(this.metrics.responses.min, duration);
                this.metrics.responses.max = Math.max(this.metrics.responses.max, duration);
                this.metrics.responses.average = 
                    (this.metrics.responses.average * (this.metrics.responses.total - 1) + duration) / 
                    this.metrics.responses.total;
    ...
    private collectMetrics(): void {
        ...
        // Request rate
        const requestDiff = this.metrics.requests.total - this.metrics.requests.lastCount;
        this.metrics.requests.perSecond = requestDiff / (this.config.interval / 1000);
    ...
    public getHealthStatus(): any {
        const health = {
            status: 'healthy',
            checks: {
                memory: this.metrics.memory.percentage < 80 ? 'healthy' : 'warning',
                responseTime: this.metrics.responses.average < 1000 ? 'healthy' : 'warning',

...
import { bufferToBase32 } from "../utils/encoding";
...
export class FortifyJS {
    ...
    public static generateSecureToken(
        ...
        return SecureRandom.generateSecureToken(length, {
            includeUppercase,
            includeLowercase,
            includeNumbers,
            includeSymbols,
    ...
    public static generateAPIKey(options: APIKeyOptions | string = {}): string {
        ...
        const randomPart = SecureRandom.generateSecureToken(randomPartLength, {
            includeUppercase: true,
            includeLowercase: true,
            includeNumbers: true,
            includeSymbols: false,
    ...
    public static generateSessionToken(
        ...
        // Generate nonce
        const nonce = SecureRandom.generateSecureToken(16, {
        ...
        if (userAgent) {
            ...
            const uaHash = Hash.secureHash(userAgent, {
                algorithm: "sha256",
                outputFormat: "hex",
            }).substring(0, 16);
        ...
        const signature = Hash.secureHash(signatureInput, {
            algorithm: "sha256",
            iterations: 1,
            outputFormat: "base64",
        });
    ...
    public static secureHash(
        ...
        return Hash.secureHash(input, options);
    }
    ...
    public static verifyHash(
        ...
        return Hash.verifyHash(input, hash, options);
    }
     ...
     */
    public static runSecurityTests(): SecurityTestResult {
        ...
        return (req: any, res: any, next: any) => {
            try {
                ...
                if (opts.rateLimit) {
                    ...
                    if (
                        !ipRequests[clientIp] ||
                        ipRequests[clientIp].resetTime < now
                    ) {
                        ipRequests[clientIp] = {
                            count: 0,
                    ...
                    if (
                        ipRequests[clientIp].count > opts.maxRequestsPerMinute
                    ) {
                        if (secureLogger) {
                            secureLogger.warning("Rate limit exceeded", {
                                ip: clientIp,
                                count: ipRequests[clientIp].count,
                        ...
                        res.status(429).json({
                            ...
                            retryAfter: Math.ceil(
                                (ipRequests[clientIp].resetTime - now) / 1000
                            ),
                        });
                        return;
                    }
                ...
                if (
                    opts.csrfProtection &&
                    !opts.excludePaths.includes(req.path)
                ) {
                    ...
                    const newToken = FortifyJS.generateSecureToken({
                        length: 32,
                    });

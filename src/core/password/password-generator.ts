...
export class PasswordGenerator {
     ...
     */
    public generate(options: PasswordGenerationOptions = {}): PasswordGenerationResult {
            ...
            if (strength.score >= opts.minStrengthScore!) {
                return {
                    ...
                    metadata: {
                        ...
                        algorithm: "fortify-secure-random",
        ...
        throw new Error("Unable to generate password meeting strength requirements");
    }
     ...
     */
    public generatePassphrase(options: {
        wordCount?: number;
        separator?: string;
        ...
        while (attempts < maxAttempts) {
            ...
            for (let i = 0; i < opts.wordCount; i++) {
                const randomIndex = SecureRandom.getSecureRandomInt(0, words.length - 1);
            ...
            if (opts.includeNumbers) {
                const number = SecureRandom.getSecureRandomInt(10, 999);
            ...
            if (opts.includeSymbols) {
                const symbols = "!@#$%^&*";
                const randomSymbol = symbols[SecureRandom.getSecureRandomInt(0, symbols.length - 1)];
        ...
        throw new Error("Unable to generate passphrase meeting strength requirements");
    }
     ...
     */
    public generateMemorable(options: {
        length?: number;
        includeNumbers?: boolean;
        includeSymbols?: boolean;
        minStrengthScore?: number;
        ...
        while (attempts < maxAttempts) {
            ...
            for (let i = 0; i < opts.length - 2; i++) {
                if (useConsonant) {
                    password += consonants[SecureRandom.getSecureRandomInt(0, consonants.length - 1)];
                } else {
                    password += vowels[SecureRandom.getSecureRandomInt(0, vowels.length - 1)];
                }
            ...
            // Add numbers if requested
            if (opts.includeNumbers) {
                password += numbers[SecureRandom.getSecureRandomInt(0, numbers.length - 1)];
            }

            // Add symbols if requested
            if (opts.includeSymbols) {
                password += symbols[SecureRandom.getSecureRandomInt(0, symbols.length - 1)];
            }
            ...
            password = password
                .split('')
                .map(char => {
                    if (char.match(/[a-z]/) && SecureRandom.getSecureRandomBoolean()) {
        ...
        throw new Error("Unable to generate memorable password meeting strength requirements");
    }

    // ===== PRIVATE HELPER METHODS =====

    private mergeOptions(options: PasswordGenerationOptions): Required<PasswordGenerationOptions> {
        return {
            length: 16,
    ...
    private generatePassword(options: Required<PasswordGenerationOptions>): string {
        let charset = "";
        ...
        for (let i = 0; i < options.length; i++) {
            const randomIndex = SecureRandom.getSecureRandomInt(0, charset.length - 1);
    ...
    private ensureAllCharacterTypes(password: string, options: Required<PasswordGenerationOptions>): string {
        ...
        if (options.includeUppercase && !/[A-Z]/.test(password)) {
            ...
            const randomIndex = SecureRandom.getSecureRandomInt(0, chars.length - 1);
            chars[randomIndex] = upperChars[SecureRandom.getSecureRandomInt(0, upperChars.length - 1)];
        }
        ...
        if (options.includeLowercase && !/[a-z]/.test(password)) {
            ...
            const randomIndex = SecureRandom.getSecureRandomInt(0, chars.length - 1);
            chars[randomIndex] = lowerChars[SecureRandom.getSecureRandomInt(0, lowerChars.length - 1)];
        }

        if (options.includeNumbers && !/[0-9]/.test(password)) {
            const numberChars = options.excludeSimilar ? "23456789" : "0123456789";
            const randomIndex = SecureRandom.getSecureRandomInt(0, chars.length - 1);
            chars[randomIndex] = numberChars[SecureRandom.getSecureRandomInt(0, numberChars.length - 1)];
        }

        if (options.includeSymbols && !/[^A-Za-z0-9]/.test(password)) {
            const symbolChars = "!@#$%^&*()_+-=[]{}|;:,.<>?";
            const randomIndex = SecureRandom.getSecureRandomInt(0, chars.length - 1);
            chars[randomIndex] = symbolChars[SecureRandom.getSecureRandomInt(0, symbolChars.length - 1)];
        }

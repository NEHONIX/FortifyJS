/**
 * Security Utilities for FortifyJS
 ...
 */
export function generateSecureRandom(size: number = SECURITY_CONSTANTS.SECURE_RANDOM_BYTES): Buffer {
    if (size <= 0 || size > 1024) {
...
export function validatePasswordStrength(password: string): {
    ...
    if (password.length < 8) {
        ...
        issues.push(`Password should be at least ${SECURITY_CONSTANTS.MIN_PASSWORD_LENGTH} characters`);
        score += 10;
    } else {
        score += 25;
    }
 ...
 */
export function validateKeyStrength(keySize: number, algorithm: string): SecurityAssessment {
    ...
    if (algorithm.toLowerCase().includes('rsa')) {
        if (keySize < 2048) {
            ...
            vulnerabilities.push('RSA key size may be vulnerable to future attacks');
            score -= 30;
            level = 'standard';
            recommendations.push('Consider upgrading to 3072+ bits');
        } else if (keySize >= 4096) {
            level = 'maximum';
            recommendations.push('Excellent key size for high-security applications');
        } else {
            level = 'high';
        }
...
export class SecurityRateLimiter {
    ...
    constructor(maxAttempts: number = 5, windowMs: number = 300000) { // 5 minutes default
...
export function validateSecureConfiguration(config: any): {
    ...
    if (config.keySize && config.keySize < SECURITY_CONSTANTS.MIN_KEY_SIZE) {
        issues.push(`Key size ${config.keySize} is below minimum secure threshold`);
        score -= 40;
        recommendations.push(`Use at least ${SECURITY_CONSTANTS.MIN_KEY_SIZE}-bit keys`);
    }

    // Check for weak algorithms
    const weakAlgorithms = ['md5', 'sha1', 'des', 'rc4'];
    for (const [key, value] of Object.entries(config)) {
        if (typeof value === 'string' && weakAlgorithms.some(weak => 

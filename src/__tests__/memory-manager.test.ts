...
async function testBasicMemoryTracking(): Promise<void> {
    ...
    console.log(`  - Total allocated: ${MemoryUtils.formatBytes(stats.totalAllocated)}`);
...
async function testMemoryPoolOperations(): Promise<void> {
    ...
    console.log(`  - LIFO Pool hit rate: ${(lifoStats.hitRate * 100).toFixed(1)}%`);
    console.log(`  - FIFO Pool hit rate: ${(fifoStats.hitRate * 100).toFixed(1)}%`);
...
async function testEventSystem(): Promise<void> {
    ...
    memoryManager.on(MemoryEventType.GC_COMPLETED, (event) => {
        console.log(`  - GC completed: freed ${MemoryUtils.formatBytes(event.data?.freedMemory || 0)}`);
    });
...
async function testConfigurationManagement(): Promise<void> {
    ...
    console.log(`  - Original GC threshold: ${(originalConfig.gcThreshold * 100).toFixed(1)}%`);
    ...
    console.log(`  - Updated GC threshold: ${(updatedConfig.gcThreshold * 100).toFixed(1)}%`);
...
async function testLeakDetection(): Promise<void> {
    ...
    // Clean up
    leakyObjects.forEach((_, i) => memoryManager.removeReference(`leak-obj-${i}`));
...
async function testGarbageCollection(): Promise<void> {
    ...
    console.log(`  - Memory before GC: ${MemoryUtils.formatBytes(beforeStats.currentUsage)}`);
    
    // Force garbage collection
    const gcResult = memoryManager.forceGC();
    
    console.log(`  - GC duration: ${gcResult.duration}ms`);
    console.log(`  - Memory freed: ${MemoryUtils.formatBytes(gcResult.freedMemory)}`);
    console.log(`  - GC successful: ${gcResult.success}`);
    
    const afterStats = memoryManager.getStats();
    console.log(`  - Memory after GC: ${MemoryUtils.formatBytes(afterStats.currentUsage)}`);
...
async function testUtilityFunctions(): Promise<void> {
    ...
    console.log(`  - Format bytes: ${MemoryUtils.formatBytes(1024 * 1024 * 2.5)}`);
    ...
    console.log(`  - Estimated object size: ${MemoryUtils.formatBytes(estimatedSize)}`);
    
    // Test feature support
    console.log(`  - Advanced features supported: ${MemoryUtils.supportsAdvancedFeatures()}`);
    
    // Test memory usage (Node.js only)
    const memUsage = MemoryUtils.getCurrentMemoryUsage();
    if (memUsage) {
        console.log(`  - Current heap usage: ${MemoryUtils.formatBytes(memUsage.heapUsed)}`);
    }

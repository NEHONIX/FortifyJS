...
console.log("🧪 Starting FortifyJS CORS Tests...");
...
async function testBasicCors() {
    ...
    try {
        ...
        const preflightResponse = await makeRequest(`http://localhost:${testPort}/cors-test`, {
            method: "OPTIONS",
            headers: {
        ...
        assert(preflightResponse.status === 204, "Preflight request should return 204");
        assert(
            preflightResponse.headers["access-control-allow-origin"] === "http://localhost:3000",
            "Should allow the specified origin"
        );
        assert(
            preflightResponse.headers["access-control-allow-methods"]?.includes("GET"),
            "Should allow GET method"
        );

        // Test actual request with allowed origin
        const actualResponse = await makeRequest(`http://localhost:${testPort}/cors-test`, {
            method: "GET",
            headers: {
                "Origin": "http://localhost:3000"
            }
        ...
        assert(
            actualResponse.headers["access-control-allow-origin"] === "http://localhost:3000",
...
async function testCorsOriginFiltering() {
    ...
    try {
        ...
        const allowedResponse = await makeRequest(`http://localhost:${testPort}/origin-test`, {
            method: "OPTIONS",
            headers: {
                "Origin": "https://allowed.com",
                "Access-Control-Request-Method": "GET"
        ...
        assert(
            allowedResponse.headers["access-control-allow-origin"] === "https://allowed.com",
            "Should allow the whitelisted origin"
        );

        // Test with disallowed origin
        const disallowedResponse = await makeRequest(`http://localhost:${testPort}/origin-test`, {
            method: "OPTIONS",
            headers: {
        ...
        assert(
            ...
            disallowedResponse.headers["access-control-allow-origin"] !== "https://malicious.com",
...
async function testCorsMethodHeaders() {
    ...
    try {
        // Test allowed method
        const allowedMethodResponse = await makeRequest(`http://localhost:${testPort}/method-test`, {
            method: "OPTIONS",
            headers: {
                "Origin": "http://localhost:3000",
                "Access-Control-Request-Method": "GET"
        ...
        assert(
            allowedMethodResponse.headers["access-control-allow-methods"]?.includes("GET"),
            "Should allow GET method"
        );

        // Test allowed headers
        assert(
            allowedMethodResponse.headers["access-control-allow-headers"]?.includes("Content-Type"),
...
async function testDisabledCors() {
    ...
    try {
        const response = await makeRequest(`http://localhost:${testPort}/no-cors-test`, {
            method: "GET",
            headers: {
                "Origin": "http://localhost:3000"
            }

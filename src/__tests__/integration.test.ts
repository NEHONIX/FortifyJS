...
console.log("🧪 Starting FortifyJS Integration Tests...");
...
async function makeRequest(url: string, options: any = {}): Promise<{ status: number; body: string; headers: any }> {
    try {
...
async function testFullStackAPI() {
    ...
    try {
        // Test CORS preflight
        const preflightResponse = await makeRequest(`http://localhost:${testPort}/api/users`, {
            method: "OPTIONS",
            headers: {
        ...
        assert(preflightResponse.status === 204, "CORS preflight should succeed");
        assert(
        ...
        const getResponse = await makeRequest(`http://localhost:${testPort}/api/users`, {
            headers: {
                "Origin": "http://localhost:3000",
                "X-API-Key": "test-key"
            }
        ...
        assert(getResponse.body.includes("John Doe"), "Should return user data");
        assert(
        ...
        const postResponse = await makeRequest(`http://localhost:${testPort}/api/users`, {
            method: "POST",
            headers: {
                "Origin": "http://localhost:3000",
        ...
        assert(postResponse.body.includes("New User"), "Should create new user");

        // Test health endpoint
        const healthResponse = await makeRequest(`http://localhost:${testPort}/api/health`);
        assert(healthResponse.status === 200, "Health check should succeed");
        assert(healthResponse.body.includes("healthy"), "Should return healthy status");
...
async function testRateLimitingWithCors() {
    ...
    try {
        ...
        const response1 = await makeRequest(`http://localhost:${testPort}/api/limited`, {
        ...
        const response2 = await makeRequest(`http://localhost:${testPort}/api/limited`, {
            headers: { "Origin": "http://localhost:3000" }
        });
        ...
        const response3 = await makeRequest(`http://localhost:${testPort}/api/limited`, {
            headers: { "Origin": "http://localhost:3000" }
        });
        assert(response3.status === 429, "Third request should be rate limited");
...
async function testErrorHandlingWithMiddleware() {
    ...
    try {
        ...
        const normalResponse = await makeRequest(`http://localhost:${testPort}/api/normal`);
        ...
        // Test error route
        const errorResponse = await makeRequest(`http://localhost:${testPort}/api/error`);
        assert(errorResponse.status >= 400, "Error route should return error status");

        // Test that normal route still works after error
        const normalResponse2 = await makeRequest(`http://localhost:${testPort}/api/normal`);
        assert(normalResponse2.status === 200, "Normal route should still work after error");
...
async function testPerformanceWithMultipleMiddleware() {
    ...
    try {
        ...
        for (let i = 0; i < 10; i++) {
            promises.push(makeRequest(`http://localhost:${testPort}/api/performance`));
        }
        ...
        responses.forEach((response, index) => {
            assert(response.status === 200, `Request ${index + 1} should succeed`);
        });

        assert(totalTime < 5000, `Performance test should complete in reasonable time (${totalTime}ms)`);
        console.log(`✅ Processed 10 requests with multiple middleware in ${totalTime}ms`);

...
async function testBasicSecurityHeaders() {
    ...
    try {
        const response = await makeRequest(`http://localhost:${testPort}/security-test`);
...
async function testSecurityWithConfig() {
    ...
    try {
        const response = await makeRequest(`http://localhost:${testPort}/config-security-test`);
...
async function testDisabledSecurity() {
    ...
    try {
        const response = await makeRequest(`http://localhost:${testPort}/no-security-test`);
...
async function testSecurityHeadersPersistence() {
    ...
    try {
        ...
        const response1 = await makeRequest(`http://localhost:${testPort}/route1`);
        const response2 = await makeRequest(`http://localhost:${testPort}/route2`);
        const response3 = await makeRequest(`http://localhost:${testPort}/route3`, {
            method: "POST"
        });
        
        // All responses should have security headers
        [response1, response2, response3].forEach((response, index) => {
            assert(response.status === 200, `Route ${index + 1} should succeed`);
            assert(
                response.headers["x-content-type-options"] === "nosniff",
...
async function testSecurityWithOtherMiddleware() {
    ...
    try {
        const response = await makeRequest(`http://localhost:${testPort}/combined-test`, {

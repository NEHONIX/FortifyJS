...
import { fFunc, createFortifiedFunction } from './src/utils/fortified-function';

async function testBasicFFunc() {
    console.log('🧪 Testing Basic fFunc...');
    
    // Create a simple secure function
    const secureAdd = fFunc(async (a: number, b: number) => {
        return a + b;
    }, {
        auditLog: true,
...
async function testAdvancedFFunc() {
    ...
    const fortified = createFortifiedFunction(async (data: string, password: string) => {
...
async function testCachedFFunc() {
    ...
    const cachedFunc = fFunc(async (input: number) => {
...
async function runTests() {
    console.log('🚀 FortifyJS fFunc Modular Architecture Test\n');
    ...
    try {
        ...
        if (allPassed) {
            console.log('🎉 All tests passed! fFunc modular architecture is working correctly.');
        } else {
